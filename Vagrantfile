...
Vagrant.configure("2") do |config|
    config.vm.define :guest do |guest_config|
            ...
            guest_config.vm.provision :shell, :path => "/vagrant/provision/shell/enable_remote_mysql_access.sh"

            guest_config.vm.provision :shell, :path => "/vagrant/provision/shell/composer.sh"
    end
end
